...
\begin{abstract}
    ...
    granularities, and may depend on each other.  \uwds also provides a set of
...
%CDW - Remove 'Besides'. I think you are trying to establish why uwds is not like previous work but I don't think that word quite works in there. Probably ok with just removing it.
...
% JK: like software, the plural of middleware is middleware :)
% CDW: I get the analogy with first-class citizens but perhaps better to replace with something like 'but is not a great strength' or 'but is not a highly developed feature'
Traditional robotic middleware, like ROS, is not particularly well-suited to
deal with these different needs: full geometric data can be represented, but are
not first-class citizens (a basic task like displaying a 3D mesh at a dynamic
...
generates many possible future states, that in turn might require further
...
in which a tree of other nodes is parented.
...
the server by the clients whenever they actually need it (rendering for
instance).
...
%CDW - brocasted -> broadcast
This tool also supports basic object manipulations, that are broadcasted to the
...
object in the scene, while providing control regarding how many pixels should actually be
...
\begin{figure}
    ...
    the interaction on a tablet that displays an {\tt uwds view} client.}
