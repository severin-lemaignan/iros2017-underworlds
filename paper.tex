...
\author{S\'everin Lemaignan$^{1}$, Yoan Sallami$^{2}$, Christopher
Wallbridge$^{3}$,\\
Aur\'elie Clodic$^{2}$, Tony Belpaeme$^{3}$, and Rachid Alami$^{2}$%
\thanks{$^{1}$Author is with Bristol Robotics Lab, University of the West of
England, Bristol, United Kingdom
        {\tt\small severin.lemaignan@brl.ac.uk},
       $^{2}$Authors are with LAAS-CNRS, Universit\'e de Toulouse, CNRS, Toulouse, France
        {\tt\small firstname.surname@laas.fr},
       $^{3}$Authors are with CRNS, Plymouth University, Plymouth, United Kingdom
...
remain under a few hundred relatively small), the computational load to manipulate these
datastructures is however dominated by the actual processings performed by the
clients with the data. In the current implementation, scene graphs and timelines
are stored in-memory. Were they required, serialization and persistent storage
are not anticipated to be difficult to implement.
...
\begin{figure}
    ...
    \caption{Screenshot of the network topology introspection tool, with
    arbitrary examples of worlds (represented as boxes) and clients (ellipses).
    CLients are connected to the worlds either as
    \emph{readers} or \emph{providers} of data. \uwds introspection features
    make it possible to also visualise when clients were last active.}
    \label{fig|explorer}
\end{figure}
...
%    spatial relations between objects and agents: \emph{allo-centric} relations  are independent of the viewpoint, while
...
between objects.
%(Figure~\ref{fig|spatialrelations}).
...
\begin{figure}
    ...
    Clients read and generate the worlds {\tt ENV} $\rightarrow$ {\tt
    ENV\_ROBOT} $\rightarrow$ \ldots $\rightarrow$ {\tt
    HUMAN*\_PERSPECTIVE}. The last two worlds {\tt HUMAN\{1,2\}\_PERSPECTIVE}
    represent the immediate visual perspective of each of the humans, as well as
    their past visual perceptions. As such, they are the \emph{visual
...
%enables to get
%knowledge about what the customer sees (or has seen) or not and to handle the
%task accordingly. We plan also to be able to add information coming from the
%dialogue (e.g. if the customer says that he has seen a particular landmark that
%could consequently be added to his belief world). Moreover, we plan also to use
%alternative world ability given by \uwds (as explained in \ref{alter}) (e.g. to
%create a world where we put the customer in front of the desired shop to infer
%(and be able to explain him) what he will see at destination).
...
%\end{figure}
%
%\uwds is being used within the European L2TOR project~\cite{belpaeme2015l2tor}
%to implement the symbolic spatial reasoning pipeline of a system used in a large
%scale study (150 children). \uwds supports 
%easy integration with the {\tt spatial\_relations} between either an object
%localisation system that would allow for use in a physical environment
%(Figure~\ref{fig|l2torexample}) or a virtual tablet game. 
%
%The \uwds\xspace {\tt spatial\_relations} module is
%used to visualise counting tasks and teach spatial words for a second language.
%
%In the counting task the child is instructed to move toy animals into
%enclosures. The {\tt spatial\_relations} module calculates when the objects have
%been placed in the enclosure, and passes this on to another part of the system.
%The robot then teaches the children l2 words for the numbers and encourages the
%children to count along. While teaching the spatial domain the robot ask the
%children to move objects into positions related to other objects (\eg put the
%box on the shelf). The performance can be evaluated by the {\tt
%spatial\_relations} module between objects. In both tasks the robot is able to
%provide targeted feedback on the position of an animal or object as the spatial
%relation is calculated rather than just attempting to fulfil a particular
%position criteria
%
%The loose coupling provided by As well, this loose
%coupling facilitated development on multiple parts of the software individually
%across multiple international sites.
%
%%\uwds has been used within the European L2TOR project~\cite{belpaeme2015l2tor}
%%in order to conceptualise and visualise the spatial relations and visibility of
%%the physical objects that participants interact with. In one of the scenarios
%%of this project, participants are instructed by a NAO robot to manipulate a
%%number of Duplo-like animals on top of a printed background paper which serves
%%as a landscape (Figure~\ref{fig|l2torexample}). The robot instructions are
%%based on pedagogical strategies to promote spatial relation learning by object
%%manipulation. Such instructions include commands to place the elephant
%%\textit{on top} or \textit{next to} the cube, or put the giraffe
%%\textit{inside} the house.  To assess the performance, the robot and the
%%operator need to compute spatial relationships between objects from the visual
...
%\uwds also differs from the DyKnow framework~\cite{de_leng_dyknow:_2016}.
%DyKnow investigates stream reasoning as a central concept for spatial and
%temporal modelling, with a recent focus on semantic-aware auto-configurable
%systems~\cite{leng_towards_2017}. \uwds
%only distribute one type of spatial (scene graph) and temporal (timeline)
%datastructures, as explained above.
