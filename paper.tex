...
\uwds is being used within the European L2TOR project~\cite{belpaeme2015l2tor} as part of a system for a large scale study with 150 children. \uwds {\tt spatial\_relations} module is used to visualise counting tasks and teach spatial words for a second language(l2). The loose coupling provided by \uwds allows for easy integration with the {\tt spatial\_relations} between either an object localisation system that would allow for use in a physical environment (Figure~\ref{fig|l2torexample}) or a virtual tablet game. As well this loose coupling facilitated development on multiple parts of the software individually across multiple international sites.

In the counting task the child is instructed to move toy animals into enclosures. The {\tt spatial\_relations} module calculates when the robots have been placed in the enclosure. The robot then teaches the children l2 words for the numbers and encourages the children to count along. 

While teaching the spatial domain the 
