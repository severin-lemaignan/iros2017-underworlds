...
With $n$ the number of worlds and $m$ the number of operators in a network
$\mathcal{N}$, the worst-case (when every world is a parameter of every
...
the server performing $n \times m$ requests.  The space complexity is the same
(as clients own a copy of the worlds they monitor), except for mesh data whose
complexity is $O(1)$ (only the server stores the mesh data).
